#include "DataStructure/StringMap.h"
...
protected:
    ...
    void assertEmptyMap() {
        ...
        EXPECT_EQ(0u, testMap.count(StringView(testKeyFirst, testKeyLength)));
        EXPECT_EQ(0u, testMap.count(testKeyStr));
        EXPECT_TRUE(testMap.find(testKey) == testMap.end());
        EXPECT_TRUE(testMap.find(StringView(testKeyFirst, testKeyLength)) ==
                    testMap.end());
    ...
    void assertSingleItemMap() {
        ...
        EXPECT_EQ(1u, testMap.count(StringView(testKeyFirst, testKeyLength)));
...
TEST_F(StringMapTest, ConstEmptyMapTest) {
    ...
    EXPECT_EQ(0u, constTestMap.count(StringView(testKeyFirst, testKeyLength)));
    EXPECT_EQ(0u, constTestMap.count(testKeyStr));
    EXPECT_TRUE(constTestMap.find(testKey) == constTestMap.end());
    EXPECT_TRUE(constTestMap.find(StringView(testKeyFirst, testKeyLength)) ==
                constTestMap.end());
...
TEST_F(StringMapTest, SmallFullMapTest) {
    ...
    Map["zwei"] = 2;
    Map["drei"] = 3;
    EXPECT_EQ(Map.erase("drei"), true);
    EXPECT_EQ(Map.erase("eins"), true);
    Map["veir"] = 4;
    Map["funf"] = 5;
...
TEST_F(StringMapTest, StringMapEntryTest) {
    StringMap<uint32_t>::value_type* entry =
        StringMap<uint32_t>::value_type::create(
            StringView(testKeyFirst, testKeyLength), 1u);
...
TEST_F(StringMapTest, MoveOnly) {
    ...
    StringView Key = "Test";
    StringMapEntry<MoveOnly>::create(Key, MoveOnly(42))->destroy();
}

TEST_F(StringMapTest, CtorArg) {
    StringView Key = "Test";
    StringMapEntry<MoveOnly>::create(Key, Immovable())->destroy();
}
...
TEST(StringMapCustomTest, BracketOperatorCtor) {
    ...
    // Test that operator[] does not create a value when it is already in the
    // map
    CountCtorCopyAndMove::Ctor = 0;
    Map["abcd"];
